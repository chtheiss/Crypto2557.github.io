...
function updateBuffRequirement(buff, data, items){
  ...
  $progbar = $("#"+data["petid"]+"-image").parents(".block")
  ...
  for (item of items){
    if (item["name"]==$petImage.data("pet")){
      if(item["fragments"]>=330){
        ...
        getPet($petImage.data("pet")).then(function(data){
          var $unit_cell = $("#"+$petImage.data("unit"));
          ...
          for (var buff of data["pet"]["buffs"]){
            ...
            if (buff["linked_pets"] == undefined){
              ...
              updateBuffRequirement(buff, data, items);
              for (var linked_pet of buff["linked_pets"]){
                getPet(linked_pet.replaceAll(" ", "_"))
                .then(function(linked_pet_info){
