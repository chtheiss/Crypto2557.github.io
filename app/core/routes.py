...
def units():
    ...
    pets_url = os.path.join(current_app.root_path, "static/json", "pets.json")
    pets = json.load(open(pets_url))
    max_buffs = [max([len(unit["buffs"]) for key, unit in units.items() if(unit["rotation"] == i)]) for i in range(1, 15)]
    max_buffs.reverse()
    max_buffs = np.repeat(np.array(max_buffs),
        [4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4], axis=0)
    max_add_buffs = [max([len(pets[unit["pet"]]["additional_buffs"]) for key, unit in units.items() if(unit["rotation"] == i)]) for i in range(1, 15)]
